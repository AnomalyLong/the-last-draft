  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center', fontFamily: 'monospace', fontSize: 12, color: '#888' }}>
        <span style={{ color: '#e0e0e0', fontWeight: 'bold', fontSize: 15, letterSpacing: 1 }}>Team Setup</span>
      ...
      {/* Event log */}
      {log.length > 0 && (
        <div style={{ background: '#111', border: '1px solid #222', borderRadius: 4, padding: 10, fontFamily: 'monospace', fontSize: 11, maxWidth: 600 }}>
          ...
          {log.map((l, i) => <div key={i} style={{ color: '#3a8fd4', wordBreak: 'break-all' }}>{l}</div>)}
        </div>
      )}
    </div>
  );
