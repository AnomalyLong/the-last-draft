...
// ─── Root component ─────────────────────────────────────────────
export function CollectionScreenNew({ roster = [], lineup: lineupProp = {}, username = '', credits = 0, onBack, onAuction, isMobile: isMobileProp }) {
  ...
  const handleAssign = (playerId, pos) => {
    setLineup(prev => {
      const next = { ...prev };
      for (const p of POS_ORDER) { if (Number(next[p]) === playerId) delete next[p]; }
      next[pos] = playerId;
      return next;
    });
  };
